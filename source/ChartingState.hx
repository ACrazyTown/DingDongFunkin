...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	public var playSnapOnlyRight:Bool = false;

	var makeNewChart:Bool = false;
	var __song:String;
	...
	public function new(?songName:String)
	{
		...
		// ASSUME NORMAL SHOULD BE PRESENT ALWAYS
		if (Assets.exists(Paths.inst(songName)) && !Song.exists(songName, songName))
		{
			makeNewChart = true;
			__song = songName;
		}
	...
	override function create()
	{
		...
		snapText = new FlxText(20,30,0,"Snap: 1/" + snap + " (Press Control to unsnap the cursor)\nAdd Notes: 1-8 (or click)\n", 14);
		snapText.scrollFactor.set();
	
		gridBlackLine = new FlxSprite(gridBG.x + gridBG.width / 2).makeGraphic(2, Std.int(gridBG.height), FlxColor.BLACK);
		add(gridBlackLine);

		goofy = new FlxText(20, 69420, 0, "THIS CHARTER IS NOT FOR EXTERNAL USE! EXPECT BUGS :PPP", 12);
		goofy.setPosition((FlxG.width - goofy.width) - 10, (FlxG.height - goofy.height) - 10);
		goofy.scrollFactor.set();
	...
	function addSongUI():Void
	{
		...
		{
			var jsonName:String = _song.song.toLowerCase().replace(" ", "-"); // Replace any spaces with '-'
			trace("\nSong Name: " + _song.song.toLowerCase() + "\nJSON Path: " + jsonName);
			loadJson(jsonName);
		});
		...
		var diffDropdown = new FlxUIDropDownMenu(loadAutosaveBtn.x, loadAutosaveBtn.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(CoolUtil.difficultyArray, true), function(diff:String)
		{
			funnyDiffLmao = diff.toLowerCase();
		});
		diffDropdown.selectedLabel = "normal";
		...
		stepperVocalVol.value = vocals != null ? vocals.volume : 0;
		...
		var characters:Array<String> = GameData.characters;
		var gfVersions:Array<String> = GameData.gfVersions;
		var stages:Array<String> = GameData.stages;
		var noteStyles:Array<String> = GameData.noteSkins;
	...
	function addSectionUI():Void
	{
		...
		var swapNotes:FlxButton = new FlxButton(10, 190, "Swap L/R Notes", swapLeftRightNotes);
		...
		check_duet = new FlxUICheckBox(120, 30, null, null, "Is Duet?", 100);
		check_duet.name = "check_duet";
		...
		tab_group_section.add(swapNotes);
		tab_group_section.add(check_duet);
	...
	function loadSong(daSong:String):Void
	{
		...
		// so we dont load erros
		if (Assets.exists(Paths.assetPath(daSong, null, VOICES)) && Assets.exists(Paths.assetPath(daSong, null, VOICES, null, true)))
		{
			vocals = new FlxSound().loadEmbedded(Paths.voices(daSong));
			vocalsBf = new FlxSound().loadEmbedded(Paths.voices(daSong, true));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'Is Duet?':
					_song.notes[curSection].isDuet = check.checked;
			...
			curRenderedNotes.forEach(function(note:Note)
			{
				if (FlxG.sound.music.playing)
				{
					FlxG.overlap(strumLine, note, function(_, _)
					{
						...
						{
							if (playSnapOnlyRight && note.x > 160)
							{
								claps.push(note);
	...
	function swapLeftRightNotes():Void
	{
		//clearSection();
		//note[1] is NOTE DATA!!!
		var goodSec:Array<Array<Dynamic>> = [];

		for (note in _song.notes[curSection].sectionNotes)
		{
			var data:Int = Std.parseInt(note[1]);
			switch (data)
			{
				case 0: data = 3;
				case 3: data = 0;
			...
			goodSec.push([note[0], data, note[2]]);
		}

		_song.notes[curSection].sectionNotes = goodSec;

		trace("shoulda swapped notes!");
		updateGrid();
	}
	...
	private function newSection(lengthInSteps:Int = 16, mustHitSection:Bool = false, altAnim:Bool = true, isDuet:Bool = false):SwagSection
	{
		var sec:SwagSection = {
			lengthInSteps: lengthInSteps,
			bpm: _song.bpm,
	...
	private function addNote(?n:Note):Void
	{
		...
		trace("PLACING NOTE WITH TYPE: " + noteType);
	...
	{
		var funnySong:String = song.toLowerCase();
		switch (funnyDiffLmao)
		{
			case "easy":
				funnySong += "-easy";
			case "hard":
		...
		trace(funnySong);
		PlayState.SONG = Song.loadFromJson(funnySong, song.toLowerCase());
		FlxG.resetState();
	}

	public static var CHART_VERSION:String = "DONUT_1.0";
