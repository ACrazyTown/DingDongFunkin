...
class Controls extends FlxActionSet
{
		...
		for (name => action in controls.byName)
		{
			for (input in action.inputs)
			{
				if (device == null || isDevice(input, device))
					byName.get(name).add(cast input);
			}
		}
	...
	{
		var fakeKeys:Array<FlxKey> = keys.copy();
		for (key in fakeKeys)
		{
			if (key == NONE)
				fakeKeys.remove(key);
		...
		inline forEachBound(control, (action, state) -> addKeys(action, fakeKeys, state));
	}
	...
	public function loadKeyBinds(?reload:Bool = true)
	{
		removeKeyboard();
		...
		inline bindKeys(Control.UI_UP, KeyBinds.keyBinds.get("ui_up"));
		inline bindKeys(Control.UI_DOWN, KeyBinds.keyBinds.get("ui_down"));
		inline bindKeys(Control.UI_LEFT, KeyBinds.keyBinds.get("ui_left"));
		inline bindKeys(Control.UI_RIGHT, KeyBinds.keyBinds.get("ui_right"));
		inline bindKeys(Control.NOTE_UP, KeyBinds.keyBinds.get("note_up"));
		inline bindKeys(Control.NOTE_DOWN, KeyBinds.keyBinds.get("note_down"));
		inline bindKeys(Control.NOTE_LEFT, KeyBinds.keyBinds.get("note_left"));
		inline bindKeys(Control.NOTE_RIGHT, KeyBinds.keyBinds.get("note_right"));
		inline bindKeys(Control.ACCEPT, KeyBinds.keyBinds.get("accept"));
		inline bindKeys(Control.BACK, KeyBinds.keyBinds.get("back"));
		inline bindKeys(Control.PAUSE, KeyBinds.keyBinds.get("pause"));
		inline bindKeys(Control.RESET, KeyBinds.keyBinds.get("reset"));
	}
